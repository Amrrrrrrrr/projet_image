...
if __name__ == "__main__":
    ...
    MIB1_big = MIB_transform(MIB1, H12, outB=BG)
    MIB2_big = MIB_transform(MIB2, np.eye(3), outB=BG)
    ...
    plt.subplot(1, 2, 1)
    plt.imshow(I_crop)
    plt.title("Mosaïque SANS élargissement (outB=B2) -> I1 peut être coupée")
    plt.axis("off")

    plt.subplot(1, 2, 2)
    plt.imshow(I_big)
    plt.title("Mosaïque AVEC élargissement (BG=union(B2,Bw)) -> pas de coupe")
    plt.axis("off")
